!function ($) {
    ...
    var Typeahead = function (element, options) {
        ...
        var inputElements = angular.element(element).find('input');
        var inputElement = inputElements[0];
        var showAllDivElement = inputElements[1];

        this.timeout = null;

        this.$showAllElement = $(showAllDivElement);
        this.$element = $(inputElement)
        this.options = $.extend({}, $.fn.typeahead.defaults, options)
        this.matcher = this.options.matcher || this.matcher
        this.sorter = this.options.sorter || this.sorter
        this.preRender = this.options.preRender || this.preRender
        ...
        this.shown = false
        this.listen()
        this.showingLoadingImage = false
    ...
    Typeahead.prototype = {
        ...
        constructor:Typeahead, select:function () {
            ...
            var dataValueId = this.$menu.find('.active').attr('data-value-id')
            this.$element
                .val(this.updater(val, dataValueId))
                .change()
            return this.hide()
            ...
            var dataPopupId = this.$element.attr("data-popup-id");
            ...
            dataPopupElement
                .append(this.$menu)
                .css({
                    top:((top + this.$menu.height() + this.$element.height()) >= windowHeight) ? (top - this.$element.height() - this.$menu.height() ) : posY + elemHeight,
                    left:posX,
                    position:"absolute"
                });
            this.$menu.show();
...
//      })
            ...
            var ulHeight = this.$menu.height();
            var ulTop = this.$menu.scrollTop();
            var activeLiTop = active.position().top;
            var liHeight = this.$menu.find('li').height();
            ...
            if (!next.length) {
                ...
                if ((activeLiTop + liHeight) > ulHeight) {
                    ...
                    this.$menu.scrollTop(setScrollTop);
                }
            }
            ...
            if (!prev.length) {
                ...
                this.$menu.scrollTop(this.$menu.find('li').last().position().top);
            } else {
            ...
            this.$element
                ...
                .on('blur', $.proxy(this.options.blur || this.blur, this))
                .on('keypress', $.proxy(this.keypress, this))
                .on('keyup', $.proxy(this.options.keyup || this.keyup, this))
            ...
            this.$showAllElement
                .on('click', $.proxy(this.showAllElementClick, this))
...
//            }
        ...
        }, showAllElementClick:function (e) {
            this.$element.focus();
            this.lookup(e, true);
        }
    }
...
//        source:[], items:8, menu:'<ul class="typeahead dropdown-menu"></ul>', item:'<li><a href="#"></a></li>', minLength:1
        source:[], menu:'<ul class="typeahead dropdown-menu"></ul>', item:'<li><a href="#"></a></li>', minLength:1
    }
...
function Popup(options) {
    ...
    if (this.relativeElement && this.position != 'onchild') {
        this.position = null;
    }
    if (this.modal) {
...
Popup.prototype.show = function () {
    ...
    var focusElement = $(this.relativeElement).find("." + FOCUS_ELEMENT_CLASS)[0];
    if (focusElement) {
        $(focusElement).focus();
    }
};
