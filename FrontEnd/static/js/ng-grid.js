...
//	self.enableCellEdit = config.enableCellEdit || colDef.enableCellEdit;
    ...
    var pEnableCellEdit = colDef.enableCellEdit;
    if(pEnableCellEdit==undefined){
        pEnableCellEdit = config.enableCellEdit;
    }
    self.enableCellEdit = pEnableCellEdit;
...
//			var inputElement = angular.element(elementWithoutComments[0].children[0]);
            var elementWithoutComments = angular.element(elm).find('div[composite-data-container="true"]');
            if (elementWithoutComments && elementWithoutComments.length > 0) {
                var childInputElementContainer = angular.element(elementWithoutComments[0]);
                var childInputElements = angular.element(childInputElementContainer[0]).find('input');
                angular.element(childInputElements[0]).focus();
                var length = childInputElements.length;
                for (var i = 0; i < length; i++) {

                    var childInput = childInputElements[i];
                    $(childInput).attr("data-index", i);
                    angular.element(childInput).bind('blur', function () {
                        setTimeout(function () {
                            var focusedElement = angular.element(childInputElementContainer).find('input:focus');
                    ...
                    angular.element(childInput).bind('keydown', function (evt) {
                        ...
                        if (evt.keyCode == 9) {
                            if(evt.shiftKey && childIndex > 0){
                                ...
                                changeFocus = true;
                            } else if (!evt.shiftKey && childIndex < (length - 1)) {
                                childIndex = childIndex + 1;
                ...
                elementWithoutComments = angular.element(elm).find('input');
                var inputElement = angular.element(elementWithoutComments[0]);
...
angular.module("ngGrid").run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("editableCellTemplate.html",
    "<input ng-class=\"'colt' + col.index\" ng-model=\"row.entity[col.field]\" />"
  );
