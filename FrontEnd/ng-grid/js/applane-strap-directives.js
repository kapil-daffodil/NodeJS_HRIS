    ...
    function ($compile, $timeout) {
        return {
            ...
            template:"<table style='width: 100%;'>" +
                ...
                "</thead>" +
                "<tbody>" +
                "<tr style='height:25px;'  ng-class-even=\"'applane-grid-row-even'\" ng-class-odd=\"'applane-grid-row-odd'\"  ng-class=\"{'app-selected-tr':row.__key__ == currentRow.__key__}\" ng-repeat='row in view.data[view.metadata.tablequery.resource]'>" +
                            ...
                            if ($scope.col.type == 'selection') {
                                $scope.$watch('row.__selected__', function (newValue, oldValue) {
                                    ...
                                    for (var i = 0; i < selectedRowCount; i++) {
                                        ...
                                        var recordKey = record.__key__;
                                        var isEqual = angular.equals(recordKey, $scope.row.__key__);
                                        if (isEqual) {
                                            index = i;
                                            break;
                                        }
