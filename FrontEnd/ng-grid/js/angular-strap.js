    ...
    function ($rootScope, $compile, $http, $timeout, $q, $templateCache, $strapConfig) {
        var ModalFactory = function ModalFactory(config) {
            function Modal(config) {
                var options = angular.extend({ show:true }, $strapConfig.modal, config), scope = options.scope ? options.scope : $rootScope.$new(), template = options.template;

                var id = 'popup-' + scope.$id;
...
'use strict';
angular.module('$strap.directives').directive('bsTypeahead', ['$parse', '$dataModel', '$timeout', function ($parse, $dataModel, $timeout) {
    'use strict';
    return {
        restrict:'E',
        template:"<div class ='ref-container' composite-data-container=\"true\" style='background-color: white;'>" +
            "<div class='ref-input-container'>" +

            "<input class='ref-input' type = 'text'  placeholder='{{colmetadata.label}}' ng-init='getValue=getColumnValue(row,colmetadata,false)'  ng-model='getValue' data-popup-id='app_popup' />" +

            "</div>" +
            "<input type='button' class='ref-button' tabindex=-1>" +
            "</div>" +
            "</div>",
        scope:true,
        replace:true,
            ...
            element.typeahead({
                source:function (query, callBack) {
                    ...
                    this.timeout = $timeout(function () {
                        var displayExpression = scope.colmetadata.columns[0].expression;
                        ...
                        var resourceQuery = {"table":scope.colmetadata.table, "columns":scope.colmetadata.columns};
                        var filter;
                        ...
                        filter[displayExpression] =query;
                ...
                preRender:function (items) {
                    ...
                    items = $(items).map(function (i, item) {
                        ...
                        if (angular.isObject(item)) {
                            itemId = item[KEY];
                            item = item[scope.colmetadata.columns[0].expression];
                        }
                ...
                updater:function (value, id) {
                    scope.$apply(function () {
                        ...
                        if ($dataModel.isNullOrUndefined(id)) {
                            ...
                            updatedValue[scope.colmetadata.columns[0].expression] = value;
                            updatedValue[KEY]= id;
                        }

                        var multiple = scope.colmetadata.multiple;
                        if (multiple) {
...
angular.module('$strap.directives').directive('bsMultiRefrenceTypeahead', ['$parse', '$dataModel', '$timeout', function ($parse, $dataModel, $timeout) {
    'use strict';
    return {
        restrict:'E',
        template:"<div>" +

            "<div style='display:table-cell;right:20px;left: 0px;'>" +

            "<div ng-repeat='option in row.businessfunction_id' style='float: left;'>" +
            "<app-multiple-refrence></app-multiple-refrence>" +
            "</div>" +

            "<input  type = 'text' placeholder='{{colmetadata.label}}' ng-model='option.name' size='{{colmetadata.label.length}}'  data-popup-id='app_popup' style='margin-top:4px;border: none;float: left;'/>" +
            "</div>" +

            "<div style='display:table-cell;width:29px;vertical-align: middle;'>" +
...
//            }
//            $(element).width(width);
//            var html = "<input type='text'  class='app-multi-refrence-down-arrow' tabindex=-1>";
//            $(html).insertAfter(element);
            ...
            element.typeahead({
                source:function (query, callBack) {
                    ...
                    var selectColumn = scope.colmetadata;
                    if (selectColumn.values) {
                        callBack(selectColumn.values);
                        return;
                    }
                    var items;
                    ...
                    this.timeout = $timeout(function () {
                        var displayExpression = scope.colmetadata.columns[0];
                        var filter = displayExpression + " like {value}";
                        var mainQuery = scope.view.metadata;
                        var mainQueryParams;
                        if (mainQuery) {
                            mainQueryParams = mainQuery.parameters;
                        }
                        if (!mainQueryParams) {
                            mainQueryParams = {};
                        }
                        ...
                        var params = angular.extend(mainQueryParams, {"value":query + "%"});

//                        var resourceQuery = {"resource":scope.colmetadata.refferedresource, "columns":scope.colmetadata.display_column};
                        var resourceQuery = {"resource":scope.colmetadata.table, "columns":scope.colmetadata.columns};

                        if (scope.colmetadata.filters) {
                            resourceQuery.filters = scope.colmetadata.filters
                        } else {
                            resourceQuery.filters = [];
                        }

                        resourceQuery.filters.push(filter)
                        resourceQuery.parameters = params;

                        if (!that.showingLoadingImage) {
                            var loadingHtml = '<img src="images/loading.gif" class="app-multi-refrence-loading-image">';
//                            $(loadingHtml).insertAfter(that.$element);
                        ...
                        $dataModel.getData(scope.componentid, resourceQuery, false, function (data) {
                            ...
                            if (query == that.$element.val()) {
                                var optionData = data[resourceQuery.resource];
                ...
                preRender:function (items) {
                    ...
                    items = $(items).map(function (i, item) {
                        ...
                        if (angular.isObject(item)) {
                            itemId = item[KEY];
                            item = item[scope.colmetadata.display_column[0]];
                        }
                ...
                updater:function (value, id) {
                    ...
                    scope.$apply(function () {
                        ...
                        if ($dataModel.isNullOrUndefined(id)) {
//                            updatedValue = value;
                            updatedValue[scope.colmetadata.display_column[0]] = value;
                        } else {
                            updatedValue[scope.colmetadata.display_column[0]] = value;
                            updatedValue[KEY] = id;
                        }
