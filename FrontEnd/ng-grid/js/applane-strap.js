...
var APPLANE_DEVELOPER_ASK = "0530db61-c658-4864-856b-6747e3a00476";
            ...
            return  {
                post:function ($scope, iElement) {
                    ...
                    $scope.onMenuClick = function (menu) {
                        ...
                        if (table) {
                            $appDomService.addView({"viewid":table.id, "applicationid":menu.applicationid.id, "ask":menu.applicationid.oauthkey, "osk":$scope.currentorganization.oauthkey}, $scope);
                        } else {
                            var view = menu.view;
                            if (!view) {
                ...
                pre:function ($scope, iElement) {
                    $scope.onActionCollectionOptionClick = function () {
                        if ($scope.action.callback) {
                            $scope[$scope.action.callback]($scope.option);
                        }
                    }
                },
                post:function ($scope, iElement) {
                ...
                pre:function ($scope, iElement, attrs) {
                    ...
                    $scope.childview = function ($event) {
                        ...
                        var columns = [
                            ...
                            ], "width":200, "table":"resources", "visibility":"Table"},
                        ...
                        metadata.savecallback = function (childs) {
                            ...
                            var param = {"ask":$scope.view.ask, "osk":$scope.currentorganization.oauthkey, "customization":JSON.stringify({"childs":childs})};
                    ...
                    $scope.columnview = function ($event) {
                        ...
                        var columns = [
                            ...
                            {"label":"Visibility", "expression":"visibility", "type":"lookup", "width":100, "options":["Both", "Table", "Panel", "None"], "visibility":"Table"}
                        ];
                        ...
                        metadata.savecallback = function (customization) {
                            var url = "http://backend.applane.com/applanestudio/" + $scope.view.applicationid + "/view/" + $scope.view.viewid + "/customization";
                            var param = {"ask":$scope.view.ask, "osk":$scope.currentorganization.oauthkey, "customization":JSON.stringify({"columns":customization})};
                            $appService.getDataFromJQuery(url, param, "POST", "JSON", "Loading...", function (data) {
                                $appDomService.addView(data, $scope);
                            });
                        };
...
//                        });
                    ...
                    $scope.onRowActionClick = function (action) {
                        ...
                        if (actionType == 'detail') {
                            ...
                            if ($scope.view.metadata.type == 'table') {
                                var data = {};
                                data[$scope.view.metadata.tablequery.resource] = [currentRow];
                                var viewinfo = {};
                                var panelColumns = [];
                                ...
                                var panelQuery = $scope.view.metadata.panelquery;
                                $dataModel.initFilters(panelQuery);
                                var filters = panelQuery.filters;
                                filters.push("__key__={__key__}");
                                panelQuery.parameters = angular.copy(currentRow);
                                var panelmetadata = {"tablequery":$scope.view.metadata.tablequery, "panelquery":panelQuery, "columns":panelColumns, "type":"panel", "layout":"onecolumns"};
                                ...
                                alert("View detail in only supported in table and not in [" + $scope.view.metadata.type + "]");
                            }
                    ...
                    $scope.save = function () {
                        ...
                        if (callback) {
                            var data = $scope.view.data[$scope.view.metadata.tablequery.resource];
                            if (closeonsave) {
                                $scope.close();
                            }
                            callback(data);
            ...
            compile:function () {
                return {
                    ...
                    post:function ($scope, iElement) {
                        ...
                        if ($scope.view.metadata.quickview) {
        ...
        compile:function () {
            return {
                post:function ($scope, iElement) {
                    ...
                    var resourceName = metadata.panelquery.resource;
...
appStrapServices.factory('$appService', [
    ...
    function ($rootScope, $http, $timeout) {
        ...
        $appService.getView = function (viewInfo, callBack) {

            var viewid = viewInfo.viewid;
            if (!viewid) {
                viewid = viewInfo.table;
                if (!viewid) {
                    alert("View id or table is mandatory for open a view");
                    throw "View id or table is mandatory for open a view";
                }
                if (angular.isObject(viewid)) {
                    viewid = viewid.id;
                }
            }
            var url = "http://backend.applane.com/applanestudio/" + viewInfo.applicationid + "/view/" + viewid;


            var newViewInfo = {};
            newViewInfo.ask = viewInfo.ask;
            newViewInfo.osk = viewInfo.osk;
            var viewIdInfo = {};
            if (viewInfo.parameters) {
                viewIdInfo.parameters = viewInfo.parameters;
            }
            if (viewInfo.filters) {
                viewIdInfo.filters = viewInfo.filters;
            }
            newViewInfo.view = JSON.stringify(viewIdInfo);

            this.getDataFromJQuery(url, newViewInfo, "POST", "JSON", "Loading...", function (callBackData) {
        ...
        $appService.getUser = function (callBack) {
            ...
            this.getDataFromJQuery(APPS_STUDIO_SERVER, {"ask":"appsstudio","module":"userservice","method":"getUserState"}, "GET", "JSON", "Loading...", function (callBackData) {
                callBack(callBackData);
...
appStrapServices.factory('$dataModel', [
    ...
    function ($http, $timeout, $rootScope, $appService) {
        ...
        $dataModel.initFilters = function (query) {
...
//          model.scope.$destroy();
        ...
        $dataModel.refresh = function (componentid) {
            ...
            var query = {"table":view.metadata.table.id, "columns":view.metadata.columnsclone, "filter":view.metadata.filter, "parameters":parameters};
        ...
        $dataModel.deleteData = function (componentid) {
            ...
            if (selectedCount > 0) {
                ...
                $appService.save({"table":view.metadata.table.id, "filter":view.metadata.filter, "parameters":view.metadata.parameters, "operations":deletedItems}, view.ask, view.osk, callBack);
        ...
        $dataModel.save = function (componentid) {
            ...
            if (newInsertsCount > 0) {
                ...
                $appService.save({"table":view.metadata.table.id, "filter":view.metadata.filter, "parameters":view.metadata.parameters, "operations":newInserts}, ask, osk, callBack);
            } else {
