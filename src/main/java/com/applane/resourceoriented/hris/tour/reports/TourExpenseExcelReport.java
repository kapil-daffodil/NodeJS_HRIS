...
import com.applane.resourceoriented.hris.reports.PFAnnualReturnReportForm6ARevised;

public class TourExpenseExcelReport extends HttpServlet {
	...
	private void generateEmployeePerformanceReport(WritableWorkbook workbook, String key) throws Exception {
		if (key.length() > 2) {
			...
			JSONArray tourExpenseRecords = new PFAnnualReturnReportForm6ARevised().getPFReportArray(key);
			for (int counter = 0; counter < tourExpenseRecords.length(); counter++) {
				// int year = 0;
				int branchId = 0;
				// Date fromDate = null;
				// Date toDate = null;
				...
				if (tourExpenseRecords != null && tourExpenseRecords.length() > 0) {
					// year = Translator.integerValue(tourExpenseRecords.getJSONObject(counter).opt("yearid.name"));
					branchId = Translator.integerValue(tourExpenseRecords.getJSONObject(counter).opt("branchid"));
					firstDateInString = "" + tourExpenseRecords.getJSONObject(counter).opt("fromdate");
					lastDateInString = "" + tourExpenseRecords.getJSONObject(counter).opt("todate");
					// fromDate = Translator.dateValue(tourExpenseRecords.getJSONObject(counter).opt("fromdate"));
					// toDate = Translator.dateValue(tourExpenseRecords.getJSONObject(counter).opt("todate"));
				}

				WritableSheet sheet = workbook.createSheet(firstDateInString + " to " + lastDateInString, counter);

				JSONArray employeeTourRecords = getEmployeeTourRecords(branchId, firstDateInString, lastDateInString);
				...
				putHeaderDefault(feemergeFormat, sheet, "Employee Tour Expense Sheet for " + firstDateInString + " to " + lastDateInString, row - 4, column + 2);
	...
	private JSONArray getEmployeeTourRecords(int branchId, String firstDateInString, String lastDateInString) throws JSONException {
		String extraFilter = "";
		if (branchId != 0) {
		...
		query.put(Data.Query.FILTERS, "tourstatusid = 2 and startdate <= '" + firstDateInString + "' and enddate >= '" + firstDateInString + "'" + extraFilter);
		query.put(Data.Query.OPTION_FILTERS, "tourstatusid = 2 and startdate <= '" + lastDateInString + "' and enddate >= '" + lastDateInString + "'" + extraFilter);
		...
		columnArray.put("advanceamount_amount");
		query.put(Data.Query.COLUMNS, columnArray);
		query.put(Data.Query.FILTERS, "tourstatusid = 2 and startdate >= '" + firstDateInString + "' and enddate <= '" + lastDateInString + "'" + filters + extraFilter);
		JSONObject object = ApplaneDatabaseEngineIMPL.getApplaneDatabaseEngine().query(query);
		JSONArray rows1 = object.getJSONArray("hris_tourrequests");
