...
public class EmployeeSalaryGenerationPerformanceHead extends HttpServlet implements ApplaneTask {
	...
	private static void sendMailInCaseOfException(Exception e) {
		String traces = ExceptionUtils.getExceptionTraceMessage("EmployeeSalaryGenerationPerformanceHead Exception >> ", e);
...
//		columnArray.put("employeeid.businessfunctionid");
	...
	private static void sendMailOnCompletion() {
		String title = "Salary Generation For Performance Head Process Completed";
				...
				if (employeeSalaryComponentArray.length() > 0) {
					...
					for (int salaryComponentCounter = 0; salaryComponentCounter < employeeSalaryComponentArray.length(); salaryComponentCounter++) {
						...
						if (componentTypeId == HRISApplicationConstants.SalaryComponentTypes.BASIC) {
							// Date fromDate = Translator.dateValue(employeeSalaryComponentArray.getJSONObject(salaryComponentCounter).opt("applicablefrom"));
							Date toDate = Translator.dateValue(employeeSalaryComponentArray.getJSONObject(salaryComponentCounter).opt("applicableto"));
							String filter = "employeeid = " + employeeId + " and attendancedate >= '" + fromDateStr + "' and attendancedate <= '" + lastDateStr + "'";
							...
							for (int attendanceCounter = 0; attendanceCounter < employeeAttendanceArray.length(); attendanceCounter++) {
								...
								double shortLeaves = Translator.doubleValue(employeeAttendanceArray.getJSONObject(attendanceCounter).opt("shortleaves"));
					...
					LogUtility.writeLog("fields >> " + fields.toString());
					salaryCalculationForRegularEmployee(employeeSalaryComponentArray, employeeRecord, fields, employeeId);
				}
			} catch (Exception e) {
				String trace = ExceptionUtils.getExceptionTraceMessage(EmployeeSalaryGenerationPerformanceHead.class.getName(), e);
	...
	private static JSONArray getOldCalculatedComponents(Object employeeId, int monthId, int yearId) throws JSONException {
		...
		query.put(Data.Query.FILTERS, "employeeid = " + employeeId + " and monthid = " + monthId + " and yearid = " + yearId + " and salarycomponentid.componentcriteriaid IN(" + HRISApplicationConstants.PERFORMANCE_BASED + "," + HRISApplicationConstants.EXPERIENCE_BASED + ")");
	 ...
	 */
	public static void salaryCalculationForRegularEmployee(JSONArray employeeSalaryComponentArray, JSONObject employeeRecord, JSONObject fields, Object employeeId) throws Exception {
		...
		if (monthlyAttendanceMap.containsKey(employeeId)) {
			...
			for (int counter = 0; counter < employeeSalaryComponentArray.length(); counter++) {
				...
				// int componentTypeId = (Integer) employeeSalaryComponentRecord.opt("salarycomponentid." + SalaryComponentKinds.COMPONENT_TYPE_ID);
				...
				if (!employeeAttendanceMap.containsKey(Translator.integerValue(employeeSalaryComponentId))) {
					Double[] attendanceDetail = new Double[2];
					...
					employeeAttendanceMap.put(Translator.integerValue(employeeSalaryComponentId), attendanceDetail);
				}
				...
				fields.put("applicableFromDateInString", EmployeeSalaryGenerationPerformanceHead.getDateInString(monthFirstDate));
				fields.put("applicableToDateInString", EmployeeSalaryGenerationPerformanceHead.getDateInString(monthLastDate));
				...
				if (salaryComponentName.toString().equalsIgnoreCase("Basic")) {
					...
					componentMap.put(salaryComponentName.toString() + basicCount, object);
				}
				...
				if (salaryComponentName.toString().replaceAll(" ", "").equalsIgnoreCase("PFEmployee")) {
					...
					object[2] = employeeSalaryComponentId;
					object[3] = salaryComponentId;
					componentMap.put(salaryComponentName.toString() + pfEmployeeCount, object);
				}
	...
	private static void calculatePFAndInsert(JSONObject fields, Object employeeId, HashMap<String, Object[]> componentMap) throws JSONException {
		if (componentMap.containsKey("PFEmployee1")) {
			...
			if (grossBasicAmount <= 6500) {
				...
				// Translator.doubleValue(componentMap.get("PFEmployee1")[0]);
				...
				if (componentMap.containsKey("PFEmployee2")) {
					...
					// amount = Translator.doubleValue(componentMap.get("PFEmployee2")[0]);
	...
	public static double[] calculateRegularEmployeeComponentAmount(Object employeeId, double componentAmount, JSONObject employeeSalaryComponentRecord, Date applicableToDate, JSONArray componentExtraInformation, JSONObject fields, double incrementDateDifference, HashMap<Integer, Double[]> employeeAttendanceMap) throws JSONException {
		...
		if (presentDays > 0.0) {
			if (componentCriteriaId == HRISApplicationConstants.FIXED_BASED) {
				...
				boolean isIncrement = fields.optBoolean("increment");
				if (isIncrement) {
					calculateAttendanceBasedAmountIncremented(employeeId, componentAmount, employeeMonthlyAttandance, incrementDateDifference, fields, calculatedAmount, employeeAttendanceMap, employeeSalaryComponentId);
				}
	...
	private static void calculateAttendanceBasedAmountIncremented(Object employeeId, double componentAmount, EmployeeMonthlyAttandanceUpdateAllColumns employeeMonthlyAttandance, double incrementDateDifference, JSONObject fields, double[] calculatedAmount, HashMap<Integer, Double[]> employeeAttendanceMap, int employeeSalaryComponentId) throws JSONException {
		...
		Double[] nonPayableLeavesAbsents = employeeAttendanceMap.get(employeeSalaryComponentId);
		...
		for (int counter = 0; counter < dailyAttendance.length(); counter++) {
			...
			if (attendanceType == HRISApplicationConstants.ATTENDANCE_LEAVE) {// && paidStatus == HRISApplicationConstants.LEAVE_EWD_UN_PAID) {
				totalLeaves += 1.0;
			} else if ((attendanceType == HRISApplicationConstants.ATTENDANCE_FIRST_HALFDAY_LEAVE || attendanceType == HRISApplicationConstants.ATTENDANCE_SECOND_HALFDAY_LEAVE)) {// && paidStatus == HRISApplicationConstants.LEAVE_EWD_UN_PAID) {
				totalLeaves += 0.50;
			}
		...
		if (nonPayableLeaves > 0 && totalLeaves > 0) {
			if (nonPayableLeaves <= totalLeaves) {
		...
		if (nonPayableAbsent > 0 && totalAbsent > 0) {
			if (nonPayableAbsent <= totalAbsent) {
		...
		employeeAttendanceMap.put(employeeSalaryComponentId, nonPayableLeavesAbsents);
	...
	public static JSONArray getActiveAndReleivedEmployeeRecords(Object __key__, JSONObject fields) throws JSONException {
		...
		query.put(Data.Query.COLUMNS, columnArray);
		query.put(Data.Query.FILTERS, "employeestatusid = " + HRISApplicationConstants.EMPLOYEE_ACTIVE + " and branchid = " + fields.opt("branchId") + extraFilter);
		query.put(Data.Query.OPTION_FILTERS, "employeestatusid = " + HRISApplicationConstants.EMPLOYEE_INACTIVE + " and branchid = " + fields.opt("branchId") + " and relievingdate >= '" + fields.opt("monthFirstDateInString") + "'" + extraFilter); // + "' and relievingdate <= '" + monthLastDateInString + "'");
	...
	public static JSONArray getEmployeeSalaryComponents(String monthFirstDate, String monthLastDate, Object employeeId) throws JSONException {
		...
		query.put(Data.Query.ORDERS, orders);
		query.put(Data.Query.COLUMNS, columnArray);
		query.put(Data.Query.FILTERS, "employeeid = " + employeeId + " and applicablefrom <= '" + monthFirstDate + "' and applicableto >= '" + monthFirstDate + "' and amount > 0.0 and salarycomponentid.componentcriteriaid IN(" + HRISApplicationConstants.PERFORMANCE_BASED + "," + HRISApplicationConstants.EXPERIENCE_BASED + ")");
		query.put(Data.Query.OPTION_FILTERS, "employeeid = " + employeeId + " and applicablefrom <= '" + monthLastDate + "' and applicableto >= '" + monthLastDate + "' and amount > 0.0 and salarycomponentid.componentcriteriaid IN(" + HRISApplicationConstants.PERFORMANCE_BASED + "," + HRISApplicationConstants.EXPERIENCE_BASED + ")");
		JSONObject object = ResourceEngine.query(query);
		...
		columnArray.put("applicableto");
		query.put(Data.Query.COLUMNS, columnArray);
		query.put(Data.Query.FILTERS, "employeeid = " + employeeId + " and applicablefrom >= '" + monthFirstDate + "' and applicableto <= '" + monthLastDate + "' and amount > 0.0" + filters + " and salarycomponentid.componentcriteriaid IN(" + HRISApplicationConstants.PERFORMANCE_BASED + "," + HRISApplicationConstants.EXPERIENCE_BASED + ")");
		object = ResourceEngine.query(query);
