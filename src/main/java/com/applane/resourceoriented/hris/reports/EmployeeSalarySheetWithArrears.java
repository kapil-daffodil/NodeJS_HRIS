...
public class EmployeeSalarySheetWithArrears extends HttpServlet {
		...
		try {
			...
			if (takeSalarySheetArrayCount > 0) {
				...
				for (int counter = 0; counter < months; counter++) {
					...
					if (filter.length() == 0) {
						filter = "monthid = " + monthId + " and yearid = " + yearId + extraFilter;// + " AND employeeid=130";
					} else {
				...
				if (salarySheetArrayCount > 0) {
					...
					for (int counter = 0; counter < salarySheetArrayCount; counter++) {
						...
						int salarySheetId = salarySheetArray.getJSONObject(counter).optInt("__key__");
						...
						if (!contain) {
							...
							JSONArray arrearsArray = getArrearsArray(salarySheetId, employeeId, EmployeeSalaryGenerationServlet.getDateInString(monthFirstDate), EmployeeSalaryGenerationServlet.getDateInString(monthLastDate));
							...
							if (arrearsArray != null && arrearsArray.length() > 0) {
								for (int arrearsCounter = 0; arrearsCounter < arrearsArray.length(); arrearsCounter++) {
									String invoiceNumber1 = DataTypeUtilities.stringValue(arrearsArray.getJSONObject(arrearsCounter).opt("invoicenumber"));
	...
	private JSONArray getArrearsArray(int salarySheetId, int employeeId, Object fromDate, Object toDate) throws Exception {
		...
		query.put(Data.Query.FILTERS, "employeeid= " + employeeId + " and invoicedate >= '" + fromDate + "' and invoicedate <= '" + toDate + "' and __key__ != " + salarySheetId);
		JSONObject object;
