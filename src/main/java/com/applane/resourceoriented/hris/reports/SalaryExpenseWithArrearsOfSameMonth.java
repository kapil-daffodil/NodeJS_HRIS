...
@Path("/SalaryExpenseWithArrearsOfSameMonth")
public class SalaryExpenseWithArrearsOfSameMonth {
		...
		try {
			if (selectedKeys != null) {
				...
				if (outputStream.size() > 0) {
					...
					responseBuilder.header("Content-Disposition", "attachment; Filename=\"SalaryExpenseWithArrearsOfSameMonth" + "\"");
				...
				for (int employeeIdsCounter = 0; employeeIdsCounter < employeeIds.length(); employeeIdsCounter++) {
					...
					if (employeeDetails.containsKey(employeeId)) {
						...
						if (tdsMap.containsKey(employeeId)) {
							...
							Object newBusinessBunction = getNewBusinessfunction(employeeId, newBusinessFunctionId);
	...
	private JSONObject getSalaryDetail(String key) throws Exception {
		...
		if (resultLength > 0) {
			...
			JSONArray salarySheetArray = getSalarySheetArray(branchId, departmentId, monthId, yearId, profitcenterId);
			...
			if (salarySheetArray != null && salarySheetArray.length() > 0) {
				...
				for (Integer employeeId : employeeDetails.keySet()) {
					Object[] details = employeeDetails.get(employeeId);
					int grossAmount = (int) Translator.doubleValue(details[4]);
	...
	private JSONArray getSalarySheetArray(Object branchId, Object departmentId, Object monthId, Object yearId, Object profitcenterId) throws Exception {
		String filter = "";
		if (branchId != null) {
		...
		query.put(Data.Query.COLUMNS, columnArray);
		query.put(Data.Query.FILTERS, "monthid = " + monthId + " AND yearid = " + yearId + filter);
