...
public class BasicSalaryGreaterThen6500AndPfDeductedExcelReport extends HttpServlet {
	...
	private void graduativityReport(HttpServletRequest req, HttpServletResponse resp) {
		...
		String decompositionParameter = "attachment;filename=Gross_Salary_Grater_Then_6500_And_PF_Deducted_Report.xls";
				...
				for (int employeeCounter = 0; employeeCounter < employeeArray.length(); employeeCounter++) {
					...
					if (employeeId != null) {
						...
						if (basicComponents != null && basicComponents.length() > 0) {
							...
							double basicCalculatedAmount = 0.0;
							for (int basicComponentCounter = 0; basicComponentCounter < basicComponents.length(); basicComponentCounter++) {
								...
								basicCalculatedAmount += ((float) (amount * incrementDateDifference) / maxDayInMonthTemp);
							}
							if (basicCalculatedAmount > 6500) {
								column = 0;
								JSONArray employeeMonthlySalaryComponentsPFtype = getEmployeeMonthlySalaryComponentsPFtype(employeeId, monthId, yearId);
								double pfAmount = 0.0;
								if (employeeMonthlySalaryComponentsPFtype != null && employeeMonthlySalaryComponentsPFtype.length() > 0) {
									for (int pfCounter = 0; pfCounter < employeeMonthlySalaryComponentsPFtype.length(); pfCounter++) {
										pfAmount += Translator.doubleValue(employeeMonthlySalaryComponentsPFtype.getJSONObject(pfCounter).opt("amount"));
									}
									if (pfAmount > 0.0 && employeeStatusId == HRISApplicationConstants.EMPLOYEE_ACTIVE) {
										row++;
										...
										putHeader(numbercellformat, activeEmployeeSheet, pfAmount, row, column++);
										srNo++;
									} else if (pfAmount > 0.0) {
										inactiveRow++;
										...
										putHeader(numbercellformat, inActiveEmployeeSheet, pfAmount, inactiveRow, column++);
										inActiveSrNo++;
									}
								}
							}
	...
	private JSONArray getEmployeeMonthlySalaryComponentsPFtype(Object employeeId, int monthId, int yearId) throws JSONException {
		JSONObject query = new JSONObject();
		query.put(Data.Query.RESOURCE, "employeemonthlysalarycomponents");
		JSONArray columnArray = new JSONArray();
		columnArray.put("amount");
		query.put(Data.Query.COLUMNS, columnArray);
		query.put(Data.Query.FILTERS, "employeeid = " + employeeId + " AND salarycomponentid.salarycomponenttypeid = " + HRISApplicationConstants.SalaryComponentTypes.PF_EMPLOYEE);
		JSONObject object;
