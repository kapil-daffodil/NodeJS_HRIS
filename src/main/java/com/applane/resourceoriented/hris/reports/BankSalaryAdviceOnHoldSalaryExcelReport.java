...
public class BankSalaryAdviceOnHoldSalaryExcelReport extends HttpServlet {
	...
	private void salarySheetExcelReport(HttpServletRequest req, HttpServletResponse resp) {
		...
		String decompositionParameter = "attachment;filename=Salary_Advice_On_Hold.xls";
		...
		try {
			...
			if (salarySheetArrayCount > 0) {
				...
				for (int counter = 0; counter < salaryDetailLineItemArray.length(); counter++) {
					...
					if (((employeeAccNo != null && employeeAccNo.length() > 0) || transferSalaryId == 2) && employeeAccNo.length() > 0 && payableAmount > 0.0 && isOnHole) {
						payableAmount += 0.50;
