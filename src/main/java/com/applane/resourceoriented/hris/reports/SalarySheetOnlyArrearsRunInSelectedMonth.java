...
@Path("/SalarySheetOnlyArrearsRunInSelectedMonth")
public class SalarySheetOnlyArrearsRunInSelectedMonth {

	static ApplaneDatabaseEngine ResourceEngine = ApplaneDatabaseEngineIMPL.getApplaneDatabaseEngine();

	@GET
	@Path("/salarySheetOnlyArrearsPost")
	public Response salarySheetOnlyArrearsGet(@Context HttpServletRequest request) throws IOException {
		return salarySheetOnlyArrearsPost(request);
	}

	@POST
	@Path("/salarySheetOnlyArrearsPost")
	public Response salarySheetOnlyArrearsPost(@Context HttpServletRequest request) throws IOException {
		...
		try {
			if (selectedKeys != null) {
				...
				if (outputStream.size() > 0) {
					...
					responseBuilder.header("Content-Disposition", "attachment; Filename=\"Salary_Sheet_Only_Arrears" + "\"");
	...
	private void salarySheetExcelReport(ByteArrayOutputStream outputStream, String selectedKeys) {
		...
		try {
			...
			if (takeSalarySheetArrayCount > 0) {
				...
				String filter = "";
				filter = "invoicedate >= '" + monthFirstDateInString + "' and invoicedate <= '" + monthLastDateInString + "' AND isarrear=1" + extraFilter;// + " AND employeeid=130";
				String optionFilter = "";
				// optionFilter = "invoicedate = '" + monthFirstDateInString + "' and invoicedate <= '" + monthLastDateInString + "' AND isarrear=0" + extraFilter;// + " AND employeeid=130";
