 ...
 */
public class LicenseDateValidation implements OperationJob {
	// job define on "hris_licensedetails"
	@Override
			...
			if (validFromDateObject != null && validToDateObject != null) {
				if ((validFromDateObject instanceof String) && (validToDateObject instanceof String)) {
					...
					if (validToDate.before(validFromDate)) {
						throw new BusinessLogicException("License Valid To Date must be greater than Valid From Date.");
					}
				}
			}
