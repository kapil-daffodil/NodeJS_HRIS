...
public class OtherExpenseValidation implements OperationJob {
	@Override
	...
	private void paidAmountValidation(Record record) {
		...
		if (operation.equalsIgnoreCase("update")) {
			Object otherExpenseId = record.getValue("__key__");
			...
			if (record.has("paidamount") && paidAmountObject != null) {
				...
				if (paidAmountObject != null) {
					...
					try {
						...
						query.put(Data.Query.RESOURCE, "hris_otherexpenses");
						...
						query.put(Data.Query.FILTERS, "__key__ = " + otherExpenseId);
						JSONObject object;
						object = ResourceEngine.query(query);
						JSONArray otherExpenseArray = object.getJSONArray("hris_otherexpenses");
						int otherExpenseArrayCount = otherExpenseArray == null ? 0 : otherExpenseArray.length();
						if (otherExpenseArrayCount > 0) {
							JSONObject approveAmountObject = (JSONObject) otherExpenseArray.getJSONObject(0).opt("approveamount");
		...
		if (record.has("approveamount") && approvedAmountObject != null) {
			...
			if (approvedAmountObject != null) {
				...
				try {
					...
					columns.put(expenseAmountColumnJson);
					query.put(Data.Query.COLUMNS, columns);
					query.put(Data.Query.FILTERS, "__key__ = " + localConveyanceId);
					JSONObject object;
					object = ResourceEngine.query(query);
					JSONArray otherExpenseArray = object.getJSONArray("hris_otherexpenses");
					...
					int otherExpenseArrayCount = otherExpenseArray == null ? 0 : otherExpenseArray.length();
					if (otherExpenseArrayCount > 0) {
						JSONObject expenseAmountObject = (JSONObject) otherExpenseArray.getJSONObject(0).opt("expenseamount");
